...
export default class CoordinatorList extends Component {
  constructor(props) {
    super(props);
    let loggedIn = false
  ...
  handleUpdate = async data => {
    //data.preventDefault();
    const token = localStorage.getItem("token")
    var response = await fetch('/api/coordinatorUpdate', {
      method: 'POST',
      headers: {
    ...
    handleDelete = async data => {
      //data.preventDefault();
      const token = localStorage.getItem("token")
      var response = await fetch('/api/coordinatorDelete', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
      ...
      handleInsert = async data => {
        //data.preventDefault();
        const token = localStorage.getItem("token")
        var response = await fetch('/api/coordinatorInsert', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
        ...
        <div>
            ...
            <div className="wrapper1">
                {console.log(this.state.projectsdata)}
                    ...
                    columns={[
                    { title: "Coordinatore Name", field: "coordinator_name" },
                    { title: "Project Name", field: "project_name" },
                    { title: "Project Details", field: "project_details" },
